...
import { sentryErrorHandler, captureException } from "@/utils/sentry";

import express from "express";
import cors from "cors";
import swaggerUi from "swagger-ui-express";
import { warmupCache } from "@/middlewares/cache";
...
import {
  PlaylistRouter,
  AdminPlaylistRouter,
} from "@/modules/playlists/routes/PlaylistRouter";
...
} from "@/modules/categories/routes/CategoriesRouter";
import {
  NotificationsRouter,
  AdminNotificationsRouter,
} from "@/modules/notifications/routes/NotificationsRouter";
import { SearchRouter } from "@/modules/search/routes/SearchRouter";
import { AdminUsersRouter } from "@/modules/users/routes/UsersRouter";
import {
  WallpaperRouter,
  AdminWallpaperRouter,
} from "@/modules/wallpapers/routes/WallpaperRouter";
import {
  MangaRouter,
  AdminMangaRouter,
} from "@/modules/manga/routes/MangaRouter";
...
import {
  FileRouter,
  AdminFileRouter,
} from "@/modules/files/routes/FilesRouter";
  ...
  const server = app.listen(port, async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        console.log(error)
        console.log(
          "❌ Erro na inicialização do Redis - continuando sem cache"
        );
      }
    }, 1000);
  });
