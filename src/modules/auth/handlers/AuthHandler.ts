...
*/
  ...
  return {
    message: "Usuário criado. Verifique seu email com o código enviado.",
  };
};
...
export const updateMe = async (
  ...
  if (data.username) {
    const usernameExists = await usernameBloomFilter.checkUsernameExists(
      data.username
    );
    if (usernameExists) {
  ...
  if (categories) {
    ...
    const uniqueCategoryIds = [
      ...new Set(categories.map((cat: { id: string }) => cat.id)),
    ];
  ...
  if (languages) {
    ...
    const uniqueLanguageIds = [
      ...new Set(languages.map((lang: { id: string }) => lang.id)),
    ];
