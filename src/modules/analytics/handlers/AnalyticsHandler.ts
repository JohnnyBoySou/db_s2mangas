...
// Nova função para retornar todos os dados estruturados para o dashboard
export const getDashboardData = async ({ startDate, endDate }: DateRange) => {
    ...
    const [
        ...
        getGeneralStats(),
        getViewsByPeriod({ startDate, endDate }),
        getUsersByPeriod({ startDate, endDate }),
        getMostViewedMangas(10),
        getMostLikedMangas(10),
        getMostCommentedMangas(10),
    ...
    return {
        ...
        mangaRankings: {
            mostViewed: mostViewedMangas,
            mostLiked: mostLikedMangas,
            mostCommented: mostCommentedMangas
        },
        userStats: {
        ...
        categoryData: {
            ...
            mangaTypes: mangaTypeStats,
            mangaStatus: mangaStatusStats
        },
        // Metadados para facilitar a criação de gráficos
        chartMetadata: {
            period: {
                startDate,
                endDate
            },
            // Dados formatados para gráficos de pizza/donut
            pieChartData: {
                mangaStatus: mangaStatusStats.map(status => ({
                    label: status.status,
                    value: status.count
                })),
                mangaTypes: mangaTypeStats.map(type => ({
                    label: type.type,
                    value: type.count
                })),
                categories: categoryStats.slice(0, 10).map(category => ({
                    label: category.name,
                    value: category.mangaCount
                })),
                languages: languageStats.map(language => ({
                    label: language.code,
                    value: language.mangaCount
                }))
            },
            // Dados formatados para gráficos de linha
            lineChartData: {
                views: viewsByPeriod.map(item => ({
                    x: item.date,
                    y: item.count
                })),
                users: usersByPeriod.map(item => ({
                    x: item.date,
                    y: item.count
                }))
            },
            // Dados formatados para gráficos de barra
            barChartData: {
                mangaPopularity: mostViewedMangas.map(manga => ({
                    label: manga.title,
                    views: manga.views,
                    likes: manga.likes,
