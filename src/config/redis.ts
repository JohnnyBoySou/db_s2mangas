...
const getRedisConfig = () => {
  ...
  const baseConfig = {
    ...
    commandTimeout: 5000,
    keepAlive: 30000,
    enableOfflineQueue: false, // Desabilita fila offline para evitar retry infinito
    enableReadyCheck: false,
    retryDelayOnFailover: 0, // Desabilita retry de conexão
    maxRetriesPerRequest: 0, // Desabilita retry de comandos
...
export const initRedis = async (): Promise<boolean> => {
  try {
    ...
    if (successCount > 0) {
      logger.info(`🎉 ${successCount}/${clients.length} conexões Redis estabelecidas`);
      return true;
    } else {
      logger.warn('⚠️ Nenhuma conexão Redis estabelecida - aplicação funcionará sem cache');
