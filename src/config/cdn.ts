...
import { getCachedImage } from "@/utils/basicImageCache";
  ...
  return async (
    ...
    try {
      ...
      if (type === "images" && req.params.id) {
        ...
        const safeResolution = validResolutions.includes(resolution as string)
          ? (resolution as string)
          : "medium";

        const cachedImage = await getCachedImage(req.params.id, safeResolution);
      ...
      res.send = function (data: any) {
        if (data instanceof Buffer || typeof data === "string") {
          ...
          if (
            CDN_CONFIG.compression.enabled &&
            size > CDN_CONFIG.compression.threshold
          ) {
