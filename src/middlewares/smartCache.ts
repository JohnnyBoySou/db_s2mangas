...
export const smartCacheMiddleware = (
  ...
  return async (
    ...
    try {
      ...
      let cachedData;
      try {
        // Usar Promise.race para evitar bloqueios durante a obtenção do cache
        const cachePromise = advancedCache.getL1Cache(cacheKey);
        cachedData = await Promise.race<any>([
          cachePromise,
          new Promise<null>((resolve) => setTimeout(() => resolve(null), 500)), // Timeout de 500ms
        ]);
      } catch (cacheError) {
        logger.warn(
          `Erro ao obter cache para ${type}: ${cacheKey}`,
          cacheError
        );
      ...
      res.json = function (body: any) {
        // Salvar no cache de forma assíncrona com timeout
        setImmediate(async () => {
          try {
            const cacheOptions = {
              ttl: config.ttl,
            ...
            // Usar Promise.race para evitar bloqueios
            const setCachePromise = advancedCache.setL1Cache(
              cacheKey,
              body,
              cacheOptions
            );
...
function generateCacheKey(
  ...
  if (config.varyBy) {
    ...
    if (Object.keys(varyParams).length > 0) {
      keyParts.push(
        "query",
        JSON.stringify(varyParams, Object.keys(varyParams).sort())
      );
    }
  }
  ...
  return async (
    ...
    res.json = function (body: any) {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        setImmediate(async () => {
          try {
            const invalidatePromise = basicCache.invalidateByTags(
              tags
            );
            await Promise.race<any>([
              invalidatePromise,
...
export const imageCacheMiddleware = (
  ...
  return async (
    ...
    try {
      ...
      const cachedImage = await advancedCache.getL1Cache(cacheKey);
        ...
        if (res.statusCode === 200 && data instanceof Buffer) {
          ...
          setImmediate(async () => {
            try {
              ...
              await advancedCache.setL1Cache(cacheKey, imageData, {
                ttl: 86400, // 24 horas
                tags: ["images"],
