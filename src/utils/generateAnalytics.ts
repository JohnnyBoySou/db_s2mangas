...
export async function generateAnalytics(): Promise<AnalyticsReport> {
  try {
    ...
    // Dados de conteúdo
    const [totalChapters, totalComments, totalReviews, totalNotifications] = await Promise.all([
 ...
 */
export async function cleanOldReports(reportsDir: string, keepCount: number = 30) {
  try {
